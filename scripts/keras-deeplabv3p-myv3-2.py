...
Modified: 
    ...
    16.01.2020: same as earlier version but only change is the training and validation image set
    17.01.2020: myv3-2 was created after the myv3-1 was started training, this version does not include the scaling/
...
from keras.layers import Input, Activation, Concatenate, Add, Reshape, Dropout, BatchNormalization
...
# mirrored_strategy = tf.contrib.distribute.MirroredStrategy(devices=["/gpu:5", "/gpu:7"])...TF version non-compatible
...
def Deeplabv3(weights='pascal_voc', input_tensor=None, infer=False,
              input_shape=(183, 183, 11), classes=1, backbone='mobilenetv2',
              OS=16, alpha=1.):
    ...
    if backbone == 'xception':
        if OS == 8:
            ...
            atrous_rates = (3, 6, 9)
        x1 = Conv2D(64, (3, 3), strides=(2, 2),
                    name='entry_flow_conv1_1ak', use_bias=False, padding='same')(batches_input)

        x = BatchNormalization(name='entry_flow_conv1_1_BN')(x1)
        x = Activation('relu')(x)

        x = _conv2d_same(x, 128, 'entry_flow_conv1_2', kernel_size=3, stride=1)
    ...
    # x = Lambda(lambda x: K.tf.image.resize_bilinear(x,size=(int(np.ceil(input_shape[0]/2)), int(np.ceil(input_shape[1]/2)))), name='ak_additional_lambda_3a')(x)
    #
...
def generate_prediction_data(path):
    ...
    created on: 24.09.19
    """
    prefix = '../Merged-Sat-Imgs-Tiles/trail-data/'
    with open(path) as f:
        for line in f:
            try:
                ...
                # scaling the data
                channMin0 = Ximg[:, :, 0].min()
                channMax0 = Ximg[:, :, 0].max()
                Ximg[:, :, 0] = (Ximg[:, :, 0] - channMin0) / (1 if channMax0 == 0 else channMax0) if (
                                                                                                           channMax0 - channMin0) == 0 else (
                        channMax0 - channMin0)

                channMin1 = Ximg[:, :, 1].min()
                channMax1 = Ximg[:, :, 1].max()
                Ximg[:, :, 1] = (Ximg[:, :, 1] - channMin1) / (1 if channMax1 == 0 else channMax1) if (
                                                                                                           channMax1 - channMin1) == 0 else (
                        channMax1 - channMin1)

                channMin2 = Ximg[:, :, 2].min()
                channMax2 = Ximg[:, :, 2].max()
                Ximg[:, :, 2] = (Ximg[:, :, 2] - channMin2) / (1 if channMax2 == 0 else channMax2) if (
                                                                                                           channMax2 - channMin2) == 0 else (
                        channMax2 - channMin2)

                channMin3 = Ximg[:, :, 3].min()
                channMax3 = Ximg[:, :, 3].max()
                Ximg[:, :, 3] = (Ximg[:, :, 3] - channMin3) / (1 if channMax3 == 0 else channMax3) if (
                                                                                                           channMax3 - channMin3) == 0 else (
                        channMax3 - channMin3)

                channMin4 = Ximg[:, :, 4].min()
                channMax4 = Ximg[:, :, 4].max()
                Ximg[:, :, 4] = (Ximg[:, :, 4] - channMin4) / (1 if channMax4 == 0 else channMax4) if (
                                                                                                           channMax4 - channMin4) == 0 else (
                        channMax4 - channMin4)

                channMin5 = Ximg[:, :, 5].min()
                channMax5 = Ximg[:, :, 5].max()
                Ximg[:, :, 5] = (Ximg[:, :, 5] - channMin5) / (1 if channMax5 == 0 else channMax5) if (
                                                                                                           channMax5 - channMin5) == 0 else (
                        channMax5 - channMin5)

                channMin6 = Ximg[:, :, 6].min()
                channMax6 = Ximg[:, :, 6].max()
                Ximg[:, :, 6] = (Ximg[:, :, 6] - channMin6) / (1 if channMax6 == 0 else channMax6) if (
                                                                                                           channMax6 - channMin6) == 0 else (
                        channMax6 - channMin6)

                channMin7 = Ximg[:, :, 7].min()
                channMax7 = Ximg[:, :, 7].max()
                Ximg[:, :, 7] = (Ximg[:, :, 7] - channMin7) / (1 if channMax7 == 0 else channMax7) if (
                                                                                                           channMax7 - channMin7) == 0 else (
                        channMax7 - channMin7)

                channMin8 = Ximg[:, :, 8].min()
                channMax8 = Ximg[:, :, 8].max()
                Ximg[:, :, 8] = (Ximg[:, :, 8] - channMin8) / (1 if channMax8 == 0 else channMax8) if (
                                                                                                           channMax8 - channMin8) == 0 else (
                        channMax8 - channMin8)

                channMin9 = Ximg[:, :, 9].min()
                channMax9 = Ximg[:, :, 9].max()
                Ximg[:, :, 9] = (Ximg[:, :, 9] - channMin9) / (1 if channMax9 == 0 else channMax9) if (
                                                                                                           channMax9 - channMin9) == 0 else (
                        channMax9 - channMin9)

                channMin10 = Ximg[:, :, 10].min()
                channMax10 = Ximg[:, :, 10].max()
                Ximg[:, :, 10] = (Ximg[:, :, 10] - channMin10) / (1 if channMax10 == 0 else channMax10) if (
                                                                                                                channMax10 - channMin10) == 0 else (
                        channMax10 - channMin10)

                return (Ximg, yimg[:, :, :, 10:11])
...
# a,b=generate_arrays_from_file('../Merged-Sat-Imgs-Tiles/img-xy-list-Copy.txt')

class DataGenerator(keras.utils.Sequence):
    def __init__(self, list_IDs, labels, prefix, batch_size=1, dim=(183, 183), n_channels=11, n_classes=10,
                 shuffle=False):
        self.dim = dim
    ...
    def __len__(self):
        print('len', str(len(self.list_IDs) / self.batch_size), 'for total size', str(len(self.list_IDs)))
    ...
    def __data_generation(self, list_IDs_temp):
        try:
            ...
            for i, ID in enumerate(list_IDs_temp):
                ...
                # below [10:11] done only to extract NDVI and match dimensions of y here and in model final layer
                ytemp = np.array(tff.imread(self.prefix + labels[ID])[:, :, 10:11])
                if np.isnan(Xtemp).any() or np.isnan(ytemp).any():
                    pass  # avoid the input images that contain NaN
                else:
                    X[i] = Xtemp
                    y[i] = ytemp
...
try:
    print("calling", str(datetime.datetime.now()))
    # to change the count of tiles inputed to model training needs to be adjusted in the script scratchModelv02UNET0915EncoderDecoder.py
    #    X,y,tiles,dates=preprocess_data()
    mod = Deeplabv3(OS=8, backbone='xception')
    #    plot_model(mod, to_file='DeepLabv3p-models/model.png')
    ...
    short_model_summary = "\n".join(stringlist)
    # fmodelsum=open('DeepLabv3p-models/DeepLabv3p-summary.txt', 'w+')
    ...
    modelName = 'MyDeepLabv3p-myv3-'
    logRunNum = '2-unit'

    if not os.path.exists(modelPath + modelName + logRunNum + '.h5'):
        #################################TRAINING######################################
        myEpoch = 500
        myValSplit = 0.1
        myBatchSize = 2
        ...
        path = '../Merged-Sat-Imgs-Tiles/trail-xy-train-unit.txt'
        partition['train'] = []
        ...
        path = '../Merged-Sat-Imgs-Tiles/trail-xy-val-unit.txt'
        ...
        prefix = '../Merged-Sat-Imgs-Tiles/trail-data/'
        print("len of train", len(partition['train']))
        print("len of validation", len(partition['validation']))
        ...
        mymc = keras.callbacks.ModelCheckpoint('DL3p2-'+logRunNum+'-{epoch:03d}.h5', save_weights_only=False, period=100)

        hist = mod.fit_generator(generator=training_generator,
                                 validation_data=validation_generator,
                                 epochs=myEpoch, verbose=2)#, callbacks=[mymc])
        ...
        ###############################SAVE-MODEL######################################
        # mod.save(modelPath+modelName+logRunNum+'.h5')
        print("model saved...")
        ...
        fig.savefig('DeepLabv3p-models/Logs/myv3-loss-mae-graph' + logRunNum + '.png')

    ###############################LOAD-MODEL######################################
    else:
        mod = load_model(modelPath + modelName + logRunNum + '.h5')
        print("model loaded")

    ##################################predict######################################
    # Using func generate_prediction_data() to fetch prediction X-y instead of deriving from generator
    try:
        predXy = generate_prediction_data('../Merged-Sat-Imgs-Tiles/trail-xy-prediction.txt')
        predX, predy = predXy[0], predXy[1]
        st = datetime.datetime.now()
        predImg = mod.predict(predX, batch_size=None, steps=1)
        et = datetime.datetime.now()
        print("prediction time:", str(et - st))
        print("..."+str(predy.shape))
        print("expected[0][0]", str(predy[0][0][:10]))
        # print("expected[0][10]", str(predy[0][10][:10]))
        print(predImg.shape)
    ...
    f.savefig('DeepLabv3p-models/Logs/myv3-predImg' + logRunNum + '.png')
